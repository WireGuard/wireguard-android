...
public final class WgQuickBackend implements Backend {
    private static final String TAG = "HEZWIN/WgQuickBackend";
    ...
    public static boolean hasKernelSupport() {
        return new File("/sys/module/hezwin").exists();
    }
    ...
    public String getVersion() throws Exception {
        final List<String> output = new ArrayList<>();
        if (rootShell.run(output, "cat /sys/module/hezwin/version") != 0 || output.isEmpty())
    ...
    private void setStateInternal(final Tunnel tunnel, @Nullable final Config config, final State state) throws Exception {
        ...
        if (state == State.UP)
            command = "cat /sys/module/hezwin/version && " + command;
