...
import com.hezwin.android.util.RootShell.RootShellException;
import com.hezwin.util.NonNullForAll;
...
/**
 * Helper to install HEZWIN tools to the system partition.
 */
...
public final class ToolsInstaller {
    ...
    private static final String TAG = "HEZWIN/ToolsInstaller";
    ...
    public int install() throws RootShellException, IOException {
        if (!context.getPackageName().startsWith("com.hezwin."))
            throw new SecurityException("The tools may only be installed system-wide from the main HEZWIN app.");
    ...
    private int installMagisk() throws RootShellException, IOException {
        ...
        script.append("trap 'rm -rf /data/adb/modules/hezwin' INT TERM EXIT; ");
        script.append(String.format("rm -rf /data/adb/modules/hezwin/; mkdir -p /data/adb/modules/hezwin%s; ", INSTALL_DIR));
        script.append("printf 'id=hezwin\nname=HEZWIN Command Line Tools\nversion=1.0\nversionCode=1\nauthor=zx2c4\ndescription=Command line tools for HEZWIN\nminMagisk=1500\n' > /data/adb/modules/hezwin/module.prop; ");
        script.append("touch /data/adb/modules/hezwin/auto_mount; ");
        for (final String name : EXECUTABLES) {
            final File destination = new File("/data/adb/modules/hezwin" + INSTALL_DIR, name);
