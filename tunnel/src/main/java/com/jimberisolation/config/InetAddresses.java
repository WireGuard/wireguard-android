...
public final class InetAddresses {
    ...
    public static InetAddress parse(final String address) throws ParseException {
        if (address.isEmpty())
            throw new com.jimberisolation.config.ParseException(InetAddress.class, address, "Empty address");
        try {
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.Q)
            ...
            throw new com.jimberisolation.config.ParseException(InetAddress.class, address, e);
            ...
            if (cause instanceof IllegalArgumentException)
                throw new com.jimberisolation.config.ParseException(InetAddress.class, address, cause);
            try {
                if (WONT_TOUCH_RESOLVER.matcher(address).matches())
                    return InetAddress.getByName(address);
                else
                    throw new com.jimberisolation.config.ParseException(InetAddress.class, address, "Not an IP address");
            } catch (final UnknownHostException f) {
                throw new com.jimberisolation.config.ParseException(InetAddress.class, address, f);
            }
        }
    }
}
