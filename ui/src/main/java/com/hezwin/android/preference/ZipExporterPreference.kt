...
import com.hezwin.android.util.activity
...
class ZipExporterPreference(context: Context, attrs: AttributeSet?) : Preference(context, attrs) {
        ...
        lifecycleScope.launch {
            ...
            try {
                exportedFilePath = withContext(Dispatchers.IO) {
                    ...
                    val outputFile = downloadsFileSaver.save("hezwin-export.zip", "application/zip", true)
                    if (outputFile == null) {
    ...
    companion object {
        private const val TAG = "HEZWIN/ZipExporterPreference"
    }
}
